...
(lib.mkIf isLinux {
  ...
  xsession.windowManager.i3 = rec {
    ...
    config = {
      ...
      startup = [
        { command = "pkill picom; ${pkgs.picom}/bin/picom --backend ${originalConfig.my.picomBackend} ${lib.optional (originalConfig.my.picomBackend == "xrender") "--xrender-sync-fence"} ${if originalConfig.my.vsync then "--vsync" else "--no-vsync"}"; notification = false; }
