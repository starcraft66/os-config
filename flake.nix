  ...
  outputs = inputs@{ self, nix-darwin, nixpkgs, nixpkgs-stable, sops-nix, home-manager, nix-doom-emacs, nixos-nvidia-vgpu, devenv, ... }: let
    ...
    patchedNixpkgs = forAllPlatforms (platform: let
      ...
      in originalNixpkgs.applyPatches {
        ...
        src = nixpkgs;
        patches = [
#         (originalNixpkgs.fetchpatch { # https://github.com/NixOS/nixpkgs/pull/211469
#           url = "https://patch-diff.githubusercontent.com/raw/NixOS/nixpkgs/pull/211469.patch";
#           sha256 = "sha256-hzLTAMKnTMMrj9I3liS6AkCtat2purWE6CdlLcVt5jY=";
#         })
        ];
      });
